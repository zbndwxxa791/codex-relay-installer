...
function Ensure-CodexCli {
    ...
    $answer = Read-Host "Install Codex CLI now with the official Windows command (irm https://chatgpt.com/codex/install.ps1 | iex)? [y/N]"
    if ($answer -notmatch '^(y|yes)$') {
        Write-Warn "Skipping Codex CLI install. Install it later with: powershell -ExecutionPolicy Bypass -Command `"irm https://chatgpt.com/codex/install.ps1 | iex`""
        return
    }

    if ($DryRun) {
        Write-Step "Would run: powershell -ExecutionPolicy Bypass -Command `"irm https://chatgpt.com/codex/install.ps1 | iex`""
        return
    }

    powershell -NoProfile -ExecutionPolicy Bypass -Command "irm https://chatgpt.com/codex/install.ps1 | iex"
    if ($LASTEXITCODE -ne 0) {
        throw "Official Codex CLI installer failed. Rerun the official command or check network access to https://chatgpt.com/codex/install.ps1."
    }
}
