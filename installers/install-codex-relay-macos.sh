...
usage() {
  cat <<'EOF'
Usage: bash "install-codex-relay-macos.sh" [options]
...
ensure_codex_cli() {
  ...
  if ! answer_yes "Install Codex CLI now with the official command (curl -fsSL https://chatgpt.com/codex/install.sh | sh)?"; then
    warn "Skipping Codex CLI install. Install it later with: curl -fsSL https://chatgpt.com/codex/install.sh | sh"
    return
  fi

  if [ "$DRY_RUN" -eq 1 ]; then
    log "Would run: curl -fsSL https://chatgpt.com/codex/install.sh | sh"
    return
  fi
  ...
  die "Official Codex CLI installer failed. Rerun the official command or check network access to https://chatgpt.com/codex/install.sh."
}
...
install_node_runtime() {
  if [ "$DRY_RUN" -eq 1 ]; then
    log "Would install Node.js LTS from the official macOS pkg if npm is missing."
    return
  fi
  ...
  command -v awk >/dev/null 2>&1 || die "awk is required to resolve the latest Node.js LTS package. Install Node.js from https://nodejs.org/, then rerun this installer."
  if ! answer_yes "npm was not found. Install Node.js LTS now from the official nodejs.org macOS pkg?"; then
    die "Node.js and npm are required before Codex CLI can be installed. Install Node.js LTS, then rerun this installer."
  fi

  node_version="$(curl -fsSL https://nodejs.org/dist/index.tab | awk 'NR > 1 && $10 != "-" { print $1; exit }')"
  [ -n "$node_version" ] || die "Could not resolve the latest Node.js LTS version from nodejs.org."
  pkg_url="https://nodejs.org/dist/${node_version}/node-${node_version}.pkg"
  pkg_path="${TMPDIR:-/tmp}/node-${node_version}.pkg"
  log "Downloading Node.js LTS from: $pkg_url"
